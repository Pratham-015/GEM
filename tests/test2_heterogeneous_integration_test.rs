...
use gem::macro_layout::{MacroKind, MacroStorageLayout};
...
fn test_heterogeneous_pipeline_integration() {
    ...
    let netlist_path = manifest_dir.join("test2/gatelevel_macropreserve.gv");
    ...
    assert!(and2_count >= 15, "Expected >= 15 AIG cells for glue logic, found {}", and2_count);
    ...
    assert_eq!(layout.state_bytes() % 8, 0, "State buffer must be 64-bit aligned");
    assert_eq!(layout.io_bytes() % 8, 0, "I/O buffer must be 64-bit aligned");
    assert_eq!(layout.total_state_words % 32, 0, "State words must be warp-padded");
    assert_eq!(layout.total_io_words % 32, 0, "I/O words must be warp-padded");
