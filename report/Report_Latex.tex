...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \node[anchor=south west,text=white,align=left,text width=145mm] at ([xshift=34mm,yshift=32mm]current page.south west) {
    ...
    \textbf{Measured revision:} \code{8c48130}\\[1.5mm]
    \textbf{Report date:} 3 September 2026
  };
\end{tikzpicture}
\clearpage
...
the modified simulator is Boolean-only: 59,060 versus 59,304 simulated cycles/s,
or 1.0041$\times$ (+0.413\%).  A separate 9-partition heterogeneous stress design
scaled from 5,226 cycles/s at one block to 16,806 cycles/s at nine blocks
(3.216$\times$).  A correctness-gated representation diagnostic found that a
preserved 15-CARRY4 chain runs at only 0.117$\times$ the shredded upstream form;
this measured regression is reported rather than presented as acceleration.
...
\begin{warningbox}
\textbf{Evidence boundary.}  The primary throughput results were regenerated
from commit \code{8c48130} with clean-tree enforcement enabled.  The benchmark
JSON records the full source, netlist, partition, executable, reference-model,
and waveform hashes needed to audit the run.
\end{warningbox}
...
\midrule
Boolean heavy & 2,000 & 4 & 507 & 0/0/0 & 25.485 & 78,478 \\
DSP heavy & 2,000 & 4 & 4,464 & 32/0/0 & 94.995 & 21,054 \\
Carry heavy & 1,000 & 4 & 2,880 & 0/128/0 & 643.902 & 1,553 \\
SRL heavy & 2,000 & 4 & 576 & 0/0/128 & 47.429 & 42,168 \\
Heterogeneous small & 1,000 & 4 & 1,120 & 4/8/8 & 131.108 & 7,627 \\
Heterogeneous mixed & 1,000 & 4 & 3,520 & 16/32/32 & 277.606 & 3,602 \\
Deep dependency & 200 & 4 & 1,981 & 0/64/0 & 431.985 & 463 \\
Heterogeneous large & 200 & 4 & 8,704 & 32/128/128 & 190.577 & 1,049 \\
Occupancy stress & 1,000 & 16 & 82,385 & 1/1/1 & 59.577 & 16,785 \\
\bottomrule
\end{tabularx}
\end{table}
...
level.  Mixed scaling drops from 7,627 CPS (small) through 3,602 CPS (medium) to
1,049 CPS (large) as AIG and macro work increase.  These values demonstrate
functional scaling but do not isolate the benefit of preserving a macro.

All seven-sample coefficients of variation were below 0.90\%; mixed and large
were 0.176\% and 0.048\%, respectively.  This repeatability does not remove
systematic issues such as laptop power state.
...
\midrule
Official upstream \code{9e913f9} & 2,000 & 33.856 & 59,059.6 & 0.318\% \\
Modified Big-GEM \code{8c48130} & 2,000 & 33.776 & 59,303.7 & 0.292\% \\
\bottomrule
\end{tabularx}
\end{table}
...
\[
 \mathrm{speedup}=\frac{59{,}303.689}{59{,}059.578}=1.004133\times,
 \qquad
 \mathrm{gain}=0.4133\%.
\]
...
throughput on this workload.  A same-RTL 15-CARRY4 diagnostic passed four
independent correctness gates but measured 77,929 CPS for shredded upstream and
9,085 CPS for preserved Big-GEM: 0.1166$\times$, or an 88.3\% regression.
This combines representation and implementation effects and is not an
...
\midrule
CPS & 5,226 & 10,737 & 13,009 & 16,806 & 16,754 & 16,707 & 16,684 & 11,393/11,398 \\
\bottomrule
\end{tabular}
\end{table}

The best retained median occurs at nine blocks:
\[
 \frac{16{,}805.678}{5{,}225.969}=3.21504\times
 \quad\text{or}\quad 221.50\%\ \text{gain over one block}.
\]
...
\midrule
Exact chain & 1 & 16.65/33.33\% & 3.764\% & 70.09\% & 5.58 & 6.63/2.89 \\
Mixed & 4 & 16.67/33.33\% & 1.212\% & 75.16\% & 1.90 & 7.36/3.73 \\
Large & 4 & 16.67/33.33\% & 1.078\% & 75.59\% & 2.77 & 7.52/3.70 \\
Occupancy stress & 16 & 16.67/33.33\% & 1.306\% & 83.23\% & 15.34 & 9.72/4.06 \\
\bottomrule
\end{tabular}}
\end{table}
...
\subsection{Unmodified GEM Comparison}

\begin{table}[H]
\centering
\caption{Controlled comparison against unmodified GEM.}
\begin{tabularx}{0.78\textwidth}{Y r r}
\toprule
Implementation & CPS & Speedup \\
\midrule
Official upstream, identical Boolean workload & 59,059.6 & 1.000$\times$ \\
Our submission, identical Boolean workload & 59,303.7 & 1.0041$\times$ \\
\bottomrule
\end{tabularx}
\end{table}
...
Measured Git revision & \code{8c48130}; clean working tree at benchmark start \\
...
python3 benchmark/profile_boomerang_ncu.py \
  --workload boolean_heavy --skip-prepare
python3 benchmark/profile_boomerang_ncu.py \
  --workload upstream_boolean --skip-prepare
python3 benchmark/profile_boomerang_ncu.py \
...
python3 benchmark/profile_block_sweep.py \
  --workload occupancy_stress --repetitions 5 --warmup-runs 8
\end{verbatim}
...
\midrule
Macro-preserved CARRY4 regression & The correctness-gated diagnostic runs at 0.1166$\times$ shredded upstream; macro dispatch and barriers dominate this graph. & Measured \\
...
throughput within 0.413\% of upstream and exposes a 3.215$\times$ useful scaling
...
and register use.  The correctness-gated 15-CARRY4 representation diagnostic
measures an 88.3\% slowdown, so the submission does not demonstrate native-macro
acceleration on that graph.  This is the most important performance risk before
judging.
